...
void main(int argc, char *argv[]) {
	...
	MPI_Offset my_offset, my_next_offset, my_current_offset;
	MPI_File fh;
	MPI_Status status;
	...
	total_number_of_integers = (long long) my_size
	...
	MPI_Info_set(siox_info, "sioxCancelHints", "true");

	MPI_File_open(MPI_COMM_WORLD, file_name, MPI_MODE_RDWR | MPI_MODE_CREATE,
			siox_info, &fh);
	//MPI_File_get_info(fh, &info_out);
	//MPI_Info_get(info_out, "useSIOXLib", 1024, value, &flag);
	//if (flag) {
			//printf("User SIOX Lib is set as: %s\n", value);
	//}
